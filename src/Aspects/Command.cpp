...
Command::Command(Entity381* ent) :
		entity(ent), unitAI(NULL), orientedPhysics3D(NULL) {
}
...
void Command::GoTo(Ogre::Vector3 diff, float dt) {
	...
	float stoppingRadius = (orientedPhysics3D->maxSpeed
			* orientedPhysics3D->maxSpeed)
...
MoveTo::MoveTo(Entity381* ent, Ogre::Vector3 location) :
		Command(ent), targetLocation(location) {
}
...
Intercept::Intercept(Entity381* ent, Entity381 *target) :
		Command(ent), targetObject(target) {
}
...
void Intercept::Tick(float dt) {
	OrientedPhysics3D *targetPhysics =
			targetObject->GetAspect<OrientedPhysics3D>();
	if (orientedPhysics3D != NULL && targetPhysics != NULL) {
		float timeNeeded =
				(targetObject->position - entity->position).length()
...
bool Intercept::Done() {
return entity->position.distance(targetObject->position)
