...
WaveMgr::WaveMgr(Engine *eng) :
		Mgr(eng), enemiesPerWave(10), waveNumber(1), spawnDelay(2), timeElapsed(
				0), timeSinceLastSpawn(0) {
...
void WaveMgr::SpawnEnemy() {
	...
	Enemy * newEnemy = static_cast<Enemy *>(engine->entityMgr->CreateEntityOfTypeAtPosition(EntityType::EnemyType, spawnPosition));
...
void WaveMgr::OnEnemyKilled(Enemy * enemy) {
	...
	spawnedEnemyList.erase(std::remove(spawnedEnemyList.begin(), spawnedEnemyList.end(), enemy));
	enemiesRemaining--;
