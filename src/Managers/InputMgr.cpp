...
void InputMgr::UpdateCamera(float dt){
    ...
    cam->translate(cameraVelocity * speedScalar * dt, Ogre::Node::TransformSpace::TS_LOCAL);
    if(cam->getPosition().y < engine->gameMgr->surfaceHeight + 5){
        cam->setPosition(cam->getPosition().x, engine->gameMgr->surfaceHeight + 5, cam->getPosition().z);
    }

    //Rotation
    if(mKeyboard->isKeyDown(OIS::KC_Q)){
        cam->rotate(Ogre::Vector3(0, 1, 0), Ogre::Radian(Ogre::Degree(cameraRotationSpeed)) * speedScalar * dt,
    ...
    if(mKeyboard->isKeyDown(OIS::KC_Z)){
        cam->rotate(Ogre::Vector3(1, 0, 0), Ogre::Radian(Ogre::Degree(cameraRotationSpeed)) * speedScalar * dt,
...
void InputMgr::UpdateSelection(float dt){
    ...
    if(tab && !tab_DownLastFrame){
        ...
        engine->entityMgr->currentEntity = (engine->entityMgr->currentEntity + 1) % engine->entityMgr->entities.size();
...
bool InputMgr::mousePressed(const OIS::MouseEvent& me, OIS::MouseButtonID id){
    ...
    Ogre::Plane ground(Ogre::Vector3(1, groundHeight, 0), Ogre::Vector3(1, groundHeight, 1), Ogre::Vector3(0, groundHeight, 1));
    ...
    if(point.first){
        ...
        for(unsigned int i = 0; i < (engine->entityMgr->entities).size(); ++i){
            ...
            float distance = (pos - intersect).crossProduct(pos - cameraPos).length() / (intersect - cameraPos).length();
            if((distance < shortestDistance || shortestDistance == -1) && distance <= selectionDistanceThreshold){
